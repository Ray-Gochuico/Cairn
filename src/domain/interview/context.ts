...
export function useInterview(): InterviewContext | null {
  ...
  return useMemo(() => {
    if (!base) return null;
    return { ...base, vehicles, assetValueSnapshots, settings, holdings, tickers, dependents, properties, housingPayments, interviewAnswers };
  }, [base, vehicles, assetValueSnapshots, settings, holdings, tickers, dependents, properties, housingPayments, interviewAnswers]);
}

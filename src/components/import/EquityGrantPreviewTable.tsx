            ...
            return (
              <tr key={row.rowId} className={ROW_BG[row.status]}>
                ...
                <td className="px-3 py-2">
                  ...
                  {err('company_name') && (
                    <div className="text-xs text-destructive italic mt-0.5">{err('company_name')!.message}</div>
                  )}
                </td>
                <td className="px-3 py-2">
                  <OwnerPersonCell
                ...
                <td className="px-3 py-2 tabular-nums text-right">
                  ...
                  {err('strike_price') && (
                    <div className="text-xs text-destructive italic mt-0.5">{err('strike_price')!.message}</div>
                  )}
                </td>
                <td className="px-3 py-2 tabular-nums text-right">
                  {(r.totalShares ?? 0).toLocaleString()}
                  {err('total_shares') && (
                    <div className="text-xs text-destructive italic mt-0.5">{err('total_shares')!.message}</div>
                  )}
                </td>
                <td className="px-3 py-2 tabular-nums text-right">
                  ${(r.currentFmv ?? 0).toLocaleString()}
                  {err('current_fmv') && (
                    <div className="text-xs text-destructive italic mt-0.5">{err('current_fmv')!.message}</div>
                  )}
                </td>
                <td className="px-3 py-2">
                  <VestingScheduleCell

...
interface RowProps {
  row: PreviewRow<HoldingResolved>;
  accounts: ReadonlyArray<{ id: number; name: string }>;
  onEdit: (rowId: RowId, patch: RawRow) => void;
  onDelete: (rowId: RowId) => void;
}

// React.memo so editing one cell re-renders only that row. `accounts` is a
// stable ctx reference and the callbacks are stable store actions.
const HoldingPreviewRow = memo(function HoldingPreviewRow({ row, accounts, onEdit, onDelete }: RowProps) {

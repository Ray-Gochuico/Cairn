// ───────────────────────────────────────────────────────────────────────────
// Whole-database backup + restore (the lossless path).
//
// Round-3 E4: the legacy JSON envelope (BackupSchema/serializeBackup/
// deserializeBackup) was deleted — it round-tripped only ~13 of ~30 tables
// and its ONLY consumer was its own unit test. The functions below back up
// and restore the ENTIRE `finance.db` file via Rust commands
// (`db_backup`/`db_restore`), so every table — transactions, settings,
// scenarios, tickers, categories, learning_state, snapshots, … — is
// preserved exactly. This is the path the Settings → Data UI uses.
// ───────────────────────────────────────────────────────────────────────────

...
describe('useLoansStore in-flight de-dupe', () => {
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(LoansRepo.prototype, 'list');
    const p1 = useLoansStore.getState().load();
    const p2 = useLoansStore.getState().load();

...
describe('useDependentsStore in-flight de-dupe', () => {
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(DependentsRepo.prototype, 'list');
    const p1 = useDependentsStore.getState().load();
    const p2 = useDependentsStore.getState().load();
  ...
  it('a load() after the previous one settles triggers a fresh DB query', async () => {
    const spy = vi.spyOn(DependentsRepo.prototype, 'list');
    await useDependentsStore.getState().load();
    await useDependentsStore.getState().load();

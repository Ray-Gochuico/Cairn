...
describe('useMerchantOverridesStore in-flight de-dupe', () => {
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(MerchantOverridesRepo.prototype, 'list');
    const p1 = useMerchantOverridesStore.getState().load();
    const p2 = useMerchantOverridesStore.getState().load();
  ...
  it('a load() after the previous one settles triggers a fresh DB query', async () => {
    const spy = vi.spyOn(MerchantOverridesRepo.prototype, 'list');
    await useMerchantOverridesStore.getState().load();
    await useMerchantOverridesStore.getState().load();

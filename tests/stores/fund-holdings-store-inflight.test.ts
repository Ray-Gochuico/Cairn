...
describe('useFundHoldingsStore in-flight de-dupe', () => {
  ...
  beforeEach(async () => {
    ...
    setDatabase(db);
    useFundHoldingsStore.setState({ fundHoldings: [], isLoading: false, error: null });
  });
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(FundHoldingsRepo.prototype, 'listAll');
    const p1 = useFundHoldingsStore.getState().load();
    const p2 = useFundHoldingsStore.getState().load();
  ...
  it('a load() after the previous one settles triggers a fresh DB query', async () => {
    const spy = vi.spyOn(FundHoldingsRepo.prototype, 'listAll');
    await useFundHoldingsStore.getState().load();
    await useFundHoldingsStore.getState().load();

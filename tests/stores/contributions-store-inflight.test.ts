...
describe('useContributionsStore in-flight de-dupe', () => {
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(ContributionsRepo.prototype, 'listAll');
    const p1 = useContributionsStore.getState().load();
    const p2 = useContributionsStore.getState().load();

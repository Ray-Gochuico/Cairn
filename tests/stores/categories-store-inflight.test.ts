...
describe('useCategoriesStore in-flight de-dupe', () => {
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(CategoriesRepo.prototype, 'list');
    const p1 = useCategoriesStore.getState().load();
    const p2 = useCategoriesStore.getState().load();

...
describe('useTransactionsStore in-flight de-dupe', () => {
  ...
  it('two concurrent load() calls hit the underlying repo read only once', async () => {
    const spy = vi.spyOn(TransactionsRepo.prototype, 'list');
    const p1 = useTransactionsStore.getState().load();
    const p2 = useTransactionsStore.getState().load();
